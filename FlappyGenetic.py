...
class Bird:
    def __init__(self, Bird_Image, Bird_Mask, weights1, weights2):
        self.Bird_X = 50
        self.Bird_Y = 50
        self.Gravity = 0.75
        self.acc = 0.075
        ...
        self.Bird_Network = NeuralNetwork(self.Bird_Y, self.Bird_distance_with_pipe, self.Pipe_Y, self.weights1, self.weights2)
...
class GameCore:
    def __init__(self, Population_Number = 1):
        ...
        self.Pipe_List = [
            ...
            Pipe(460, random.randint(235, 340), 1, self.Pipe_Image),
            Pipe(620, random.randint(235, 355), 2, self.Pipe_Image),
            Pipe(780, random.randint(225, 325), 3, self.Pipe_Image),
        ]
        self.Pipe_id = 4
            ...
            if len(self.Next_Generation) < self.Population_Number:
                ...
                for a, b in zip(member_1_weights_1, member_2_weights_1):
                    for c, d in zip(a, b):
                        prob = random.random()
                        if prob < 0.47:
                            child_weights_1.append(c)
                        elif prob < 0.94:
                            child_weights_1.append(d)
                        else:
                            child_weights_1.append(random.uniform(-1, 1))

                for e, f in zip(member_1_weights_2, member_2_weights_2):
                    for g, h in zip(e, f):
                        prob = random.random()
                        if prob < 0.47:
                            child_weights_2.append(g)
                        elif prob < 0.94:
                            child_weights_2.append(h)
                        else:
                            child_weights_2.append(random.uniform(-1, 1))

                child_weights_1 = np.array(child_weights_1).reshape(3, 7)
                child_weights_2 = np.array(child_weights_2).reshape(7, 1)

                self.Next_Generation.append(Bird(self.Bird_Image, self.Bird_Mask, child_weights_1, child_weights_2))
    ...
    def GameLoop(self):
        ...
        for pipe in self.Pipe_List:
            ...
            for Member in self.Population:
                ...
                collision_lower = self.MaskCollision(Member.Bird_Mask, Member.Bird_X, Member.Bird_Y, pipe.Pipe_Lower_Mask, pipe.Pipe_X, pipe.Pipe_Lower_Y)
                collision_upper = self.MaskCollision(Member.Bird_Mask, Member.Bird_X, Member.Bird_Y, pipe.Pipe_Upper_Mask, pipe.Pipe_X, pipe.Pipe_Upper_Y)
                ...
                self.max_score = Member.Score

        print(f"Kuş Sayısı : {len(self.Population)}, Gen : {self.Num_Gen}, Score : {self.max_score}")
...
#Result (Population Number : 300)w
#Number of Birds : 25, Gen : 4, Score : 751